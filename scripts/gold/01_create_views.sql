/*
===============================================================================
Gold Layer: Business Data Model
===============================================================================

Purpose:
    Create the Gold layer of the data warehouse by transforming the cleansed
    Silver layer into a business-friendly star schema optimized for reporting
    and analytics.

Overview:
    - Integrates multiple Silver tables into unified business entities.
    - Presents data using descriptive business terminology.
    - Generates surrogate keys for dimension tables.
    - Enriches dimensions by combining CRM and ERP datasets.
    - Provides a star schema consisting of dimensions and fact tables
      for Power BI and analytical workloads.

Objects:
    - gold.dim_customers
    - gold.dim_products
    - gold.fact_sales

===============================================================================
*/

-- Customer Dimension
-- Integrates CRM customer information with ERP demographic and location information.
CREATE OR REPLACE VIEW gold.dim_customers AS
SELECT

    -- Generate a surrogate key for analytical relationships
    ROW_NUMBER() OVER (ORDER BY ci.cst_id) AS customer_key,

    ci.cst_id AS customer_id,
    ci.cst_key AS customer_number,
    ci.cst_firstname AS first_name,
    ci.cst_lastname AS last_name,

    -- Business Rule: Prioritize CRM gender when available; otherwise use ERP gender.
    CASE
        WHEN ci.cst_gndr != 'n/a' THEN ci.cst_gndr
        ELSE COALESCE(ca.gen, 'n/a')
    END AS gender,

    ci.cst_marital_status AS marital_status,
    ca.bdate AS birth_date,
    la.cntry AS country
FROM silver.crm_cust_info ci
LEFT JOIN silver.erp_cust_az12 ca
    ON ci.cst_key = ca.cid
LEFT JOIN silver.erp_loc_a101 la
    ON ci.cst_key = la.cid;


-- Product Dimension
-- Combines product information with ERP category information.
CREATE OR REPLACE VIEW gold.dim_products AS
SELECT

    -- Generate a surrogate key for analytical relationships
    ROW_NUMBER() OVER (ORDER BY pi.prd_start_dt, pi.prd_key) AS product_key,

    pi.prd_id AS product_id,
    pi.prd_cat AS category_id,
    pi.prd_key AS product_number,
    pi.prd_nm AS product_name,

    -- Data Integration: Retrieve descriptive product category attributes from ERP.
    CAST(COALESCE(pc.cat, 'n/a') AS VARCHAR(50)) AS category,
    CAST(COALESCE(pc.subcat, 'n/a') AS VARCHAR(50)) AS subcategory,
    CAST(COALESCE(pc.maintenance, 'n/a') AS VARCHAR(50)) AS maintenance,

    pi.prd_cost AS product_cost,
    pi.prd_line AS product_line,
    pi.prd_start_dt AS start_date
FROM silver.crm_prd_info pi
LEFT JOIN silver.erp_px_cat_g1v2 pc
    ON pi.prd_cat = pc.id

-- Business Rule: Include only currently active products.
WHERE pi.prd_end_dt IS NULL;


-- Sales Fact
-- Connects sales details with customer and product dimensions.
CREATE OR REPLACE VIEW gold.fact_sales AS
SELECT

    sd.sls_ord_num AS order_number,

    -- Foreign keys to dimension tables
    pr.product_key,
    cu.customer_key,

    sd.sls_order_dt AS order_date,
    sd.sls_ship_dt AS shipping_date,
    sd.sls_due_dt AS due_date,
    sd.sls_sales AS sales,
    sd.sls_quantity AS quantity,
    sd.sls_price AS price

FROM silver.crm_sales_details sd
LEFT JOIN gold.dim_customers cu
    ON sd.sls_cust_id = cu.customer_id
LEFT JOIN gold.dim_products pr
    ON pr.product_number = sd.sls_prd_key;
